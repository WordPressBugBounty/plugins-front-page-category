...
class FrontPageCats {
	...
	public function bad_post_type( $query ) {
		...
		/**
		 * If the first value does not exist then we can't be sure of the post
		 * type so quit.
		 */
		if ( is_array( $query->query['post_type'] ) && ! isset( $query->query['post_type'][0] ) ) {
			return true;
		}
		 ...
		 *
		 * If the value is not a post then we don't want to filter it.
		 */
